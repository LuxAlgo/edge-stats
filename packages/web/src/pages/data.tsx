...
export function DataPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-medium tracking-tight">Data</h1>
      ...
      <div className="mt-8 space-y-6">
        ...
        <Panel>
          <PanelHeader>symbols · last synced bar</PanelHeader>
                ...
                <table className="w-full min-w-[560px] border-collapse text-sm">
                  <thead>
                    <tr>
                      ...
                      <th className={tableHeadClass}>Adapter</th>
                      <th className={tableHeadClass}>Timeframe</th>
                      <th className={tableHeadClass}>Last bar</th>
                      <th className={`${tableHeadClass} pr-0`}>Staleness</th>
                    </tr>
                  </thead>
                  <tbody>
                    {freshness.data.symbols.map((s) => {
                      ...
                      return (
                        <tr key={s.symbol} className="border-t border-white/[0.06]">
                          <td className="stat py-2.5 pr-3 font-mono text-[13px] font-medium">
                            {s.symbol}
                          </td>
                          <td className="py-2.5 pr-3 text-muted-foreground">{s.adapter}</td>
                          <td className="stat py-2.5 pr-3 font-mono text-[13px]">{s.tf}</td>
                          <td className="stat py-2.5 pr-3 font-mono text-[13px] text-muted-foreground">
        ...
        <Panel>
          <PanelHeader>exchange calendars · versioned data files</PanelHeader>
          <div className="p-4 pt-0">
            {freshness.loading ? (
              <div className="mt-3 space-y-2">
                <Skeleton className="h-6 w-full" />
              ...
              <div className="mt-3 overflow-x-auto">
                <table className="w-full min-w-[560px] border-collapse text-sm">
                  <thead>
                    <tr>
                      <th className={tableHeadClass}>Exchange</th>
                      <th className={tableHeadClass}>Version</th>
                      <th className={tableHeadClass}>Coverage</th>
                      <th className={`${tableHeadClass} pr-0`}>Horizon</th>
                    </tr>
                  </thead>
                  <tbody>
                    {freshness.data.calendars.map((c) => {
                      ...
                      return (
                        <tr key={c.exchange} className="border-t border-white/[0.06]">
                          <td className="py-2.5 pr-3 font-mono text-[13px] font-medium">
                            {c.exchange}
                          </td>
        ...
        <Panel>
          <PanelHeader>adapters · how bars get in</PanelHeader>
          <div className="p-4 pt-0">
            {adapters.error ? (
              <div className="mt-3">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                {(adapters.data?.adapters ?? []).map((a) => (
                  ...
                  >
                    ...
                    <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">{a.doc}</p>
        ...
        {freshness.data ? (
          <div className="stat flex flex-wrap items-center gap-x-6 gap-y-1 rounded-xl border border-border bg-card px-4 py-2.5 font-mono text-[10.5px] text-faint">
